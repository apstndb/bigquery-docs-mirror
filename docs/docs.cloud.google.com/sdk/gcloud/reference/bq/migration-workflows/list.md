---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/bq/migration-workflows/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/bq/migration-workflows/list
title: gcloud bq migration-workflows list
...
`gcloud bq migration-workflows list` `  --location  ` = `  LOCATION  ` \[ `  --endpoint-mode  ` = `  ENDPOINT_MODE  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
