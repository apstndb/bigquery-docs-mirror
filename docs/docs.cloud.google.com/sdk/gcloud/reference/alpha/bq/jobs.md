---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/jobs
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/jobs
title: gcloud alpha bq jobs
description: Offers tools and libraries that allow you to create and manage resources across Google Cloud.
data_source: docs.cloud.google.com
---

NAME

gcloud alpha bq jobs - interact with and manage jobs in Google BigQuery

SYNOPSIS

`gcloud alpha bq jobs` `  GROUP  ` | `  COMMAND  ` \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(ALPHA)` Interact with and manage jobs in Google BigQuery.

GCLOUD WIDE FLAGS

These flags are available to all commands: `  --help  ` .

Run ` $ gcloud help  ` for details.

GROUPS

`  GROUP  ` is one of the following:

  - `  config  `  
    `(ALPHA)` Manage Google BigQuery job configurations.

COMMANDS

`  COMMAND  ` is one of the following:

  - `  cancel  `  
    `(ALPHA)` Cancel a BigQuery job.
  - `  describe  `  
    `(ALPHA)` Describe a BigQuery job.
  - `  list  `  
    `(ALPHA)` List all BigQuery jobs in a project.

NOTES

This command is currently in alpha and might change without notice. If this command fails with API permission errors despite specifying the correct project, you might be trying to access an API with an invitation-only early access allowlist.
