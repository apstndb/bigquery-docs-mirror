---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/tables/config/export
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/tables/config/export
title: gcloud alpha bq tables config export
...
`(ALPHA)` `gcloud alpha bq tables config export` exports the configuration for a Google BigQuery table.

Table configurations can be exported in Kubernetes Resource Model (krm) or Terraform HCL formats. The default format is `krm` .

Specifying `--all` allows you to export the configurations for all tables within the project.
...
Table resource - Table to export the configuration for. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
