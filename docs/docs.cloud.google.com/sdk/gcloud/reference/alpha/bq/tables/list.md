---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/tables/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/tables/list
title: gcloud alpha bq tables list
...
gcloud alpha bq tables list - list all BigQuery tables and views in a dataset

SYNOPSIS

`gcloud alpha bq tables list` `  --dataset  ` = `  DATASET  ` \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
Dataset resource - The BigQuery dataset you want to list datasets for. This represents a Cloud resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
