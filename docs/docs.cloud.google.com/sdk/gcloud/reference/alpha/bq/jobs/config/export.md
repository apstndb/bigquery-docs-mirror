---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/jobs/config/export
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/bq/jobs/config/export
title: gcloud alpha bq jobs config export
...
`gcloud alpha bq jobs config export` (\[ `  JOB  ` \]\] `  --all  ` ) \[ `  --path  ` = `  PATH  ` ; default="-"\] \[ `  --resource-format  ` = `  RESOURCE_FORMAT  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(ALPHA)` `gcloud alpha bq jobs config export` exports the configuration for a Google BigQuery job.
