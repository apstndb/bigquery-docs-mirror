...
1.  [Create a data policy](https://docs.cloud.google.com/bigquery/docs/column-data-masking#create-data-policies-on-column) .

2.  [Assign a data policy](https://docs.cloud.google.com/bigquery/docs/column-data-masking#assign-to-column) to a column.

### Mask data with policy tags

Figure 1 shows the workflow for configuring data masking:

![To enable data masking, you must create a taxonomy, create data policies for the policy tags in the taxonomy, and then associate the policy tags with table columns.](https://docs.cloud.google.com/static/bigquery/images/data-masking-workflow.png) **Figure 1.** Data masking components.
...
![Policy tags are associated with data policies to configure data masking, and then associated with table columns to enable masking.](https://docs.cloud.google.com/static/bigquery/images/data-masking-components.png) **Figure 2.** Data masking components.

For more information about role interaction, see [How Masked Reader and Fine-Grained Reader roles interact](https://docs.cloud.google.com/bigquery/docs/column-data-masking-intro#role-interaction) . For more information about policy tag inheritance, see [Roles and policy tag hierarchy](https://docs.cloud.google.com/bigquery/docs/column-data-masking-intro#auth-inheritance) .
...
You can configure up to nine data policies for a policy tag, each with a different data masking rule associated with it. One of these policies is reserved for [column-level access control settings](https://docs.cloud.google.com/bigquery/docs/column-level-security#set_up_column-level_access_control) . This makes it possible for several data policies to be applied to a column in a user's query, based on the groups that the user is a member of. When this happens, BigQuery chooses which data masking rule to apply based on the following hierarchy:
...
![When there is a conflict between applying the nullify and the hash (SHA-256) data masking rules due to the groups a user is in, the hash (SHA-256) data masking rule is prioritized.](https://docs.cloud.google.com/static/bigquery/images/multiple-data-policies.png)
...
When you use [BigQuery features that aren't compatible with data masking](https://docs.cloud.google.com/bigquery/docs/column-data-masking-intro#compatibility) , the service treats the masked column as a secured column, and only grants access to users who have the Data Catalog Fine-Grained Reader role.
...
After a taxonomy has a data policy associated with at least one of its policy tags, [access control](https://docs.cloud.google.com/bigquery/docs/column-level-security#enforce_access_control) is automatically enforced. If you want to turn off access control, you must first delete all of the data policies associated with the taxonomy.
...
Compatible. Data masking policies are enforced on [BigLake tables](https://docs.cloud.google.com/bigquery/docs/biglake-intro) .
