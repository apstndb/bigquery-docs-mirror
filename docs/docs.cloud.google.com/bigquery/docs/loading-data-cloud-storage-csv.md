...
For **Source column match** , choose one of the following strategies used to match the loaded columns to the schema ( [Preview](https://cloud.google.com/products#product-launch-stages) ).
...
For **Null markers** , enter a list of custom strings that represents a NULL value in CSV data. ( [Preview](https://cloud.google.com/products/#product-launch-stages) ).
...
Other optional flags include:
  ...
  - `  --null_markers  ` : ( [Preview](https://cloud.google.com/products/#product-launch-stages) ) An optional comma-separated list of custom strings that represent NULL values in CSV data. This option cannot be used with `  --null_marker  ` flag.

  - `  --source_column_match  ` : ( [Preview](https://cloud.google.com/products#product-launch-stages) ) Specifies the strategy used to match loaded columns to the schema. You can specify `  POSITION  ` to match loaded columns by position, assuming that the columns are ordered the same way as the schema. You can also specify `  NAME  ` to match by name by reading the header row as the column names and reordering columns to match the field names in the schema. If this value is unspecified, then the default is based on how the schema is provided. If `  --autodetect  ` is enabled, then the default behavior is to match columns by name. Otherwise, the default is to match columns by position.
