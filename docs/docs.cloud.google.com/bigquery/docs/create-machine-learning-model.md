...
Evaluate the performance of the model by using the [`ML.EVALUATE` function](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-evaluate) or the [`bigframes.bigquery.ml.evaluate` BigQuery DataFrames function](https://dataframes.bigquery.dev/reference/api/bigframes.bigquery.ml.evaluate.html#bigframes.bigquery.ml.evaluate) . The `ML.EVALUATE` and `bigframes.bigquery.ml.evaluate` functions evaluate the predicted values generated by the model against the actual data. To calculate logistic regression specific metrics, you can use the [`ML.ROC_CURVE` function](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-roc) .
