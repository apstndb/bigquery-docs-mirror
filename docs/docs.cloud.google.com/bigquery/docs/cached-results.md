...
title: Use cached query results
...
All query results, including both [interactive and batch queries](https://docs.cloud.google.com/bigquery/docs/running-queries) , are cached in temporary tables for approximately 24 hours with some [exceptions](https://docs.cloud.google.com/bigquery/docs/cached-results#cache-exceptions) .
...
Cross-user caching is available if you are using the Enterprise or Enterprise Plus [edition](https://docs.cloud.google.com/bigquery/docs/editions-intro) . The same [limits and exceptions](https://docs.cloud.google.com/bigquery/docs/cached-results#cache-exceptions) for single-user caching apply to cross-user caching.
...
The impact is if a user ran a query, the query results remain visible to the user. The user can also retrieve those results from the cache even if they lost access to the data within the last 24 hours.
