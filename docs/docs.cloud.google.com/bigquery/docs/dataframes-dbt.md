...
If you're authenticating using a service account, also ensure you have the [Service Account User role](https://docs.cloud.google.com/iam/docs/roles-permissions/iam#iam.serviceAccountUser) ( `roles/iam.serviceAccountUser` ) granted for the service account you plan to use.
...
The impersonated service account must also have all [roles required for authentication](https://docs.cloud.google.com/bigquery/docs/dataframes-dbt#service_account) .
...
2.  In addition to all [roles required for service account authentication](https://docs.cloud.google.com/bigquery/docs/dataframes-dbt#service_account) , grant the following roles to the service account in the credential project:
...
If you plan to authenticate to the `dbt-bigquery` adapter using OAuth but want the data processing and notebook execution to occur under the identity of a service account in a different project from which the jobs are run, ask your administrator to do the following:
  ...
  - Grant you and the service account the [roles required for service account impersonation](https://docs.cloud.google.com/bigquery/docs/dataframes-dbt#service_account_impersonation)
...
If you're using Colab Enterprise in a Shared VPC environment, ask your administrator to grant the following roles and permissions:
  ...
  - [`compute.subnetworks.use` permission](https://docs.cloud.google.com/iam/docs/roles-permissions/compute#compute.subnetworks.use) : Grant this permission to the service account used by the Colab Enterprise runtime on the host project or specific subnets. This permission is included in the [Compute Network User role](https://docs.cloud.google.com/iam/docs/roles-permissions/compute#compute.networkUser) ( `roles/compute.networkUser` ).
...
If there isn't a notebook template already available in the project and the user executing the code has the permissions to create the template, the `dbt-bigquery` adapter automatically creates and uses the default notebook template. You can also specify a different notebook template using a [dbt configuration](https://docs.cloud.google.com/bigquery/docs/dataframes-dbt#configurations) .
...
When using the `dbt-bigquery` adapter with BigQuery DataFrames, there are Google Cloud charges from the following:
  ...
  - BigQuery query execution: In the notebook, BigQuery DataFrames converts Python to SQL and executes the code in BigQuery. You are charged according to your project configuration and your query, as described for BigQuery DataFrames [pricing](https://docs.cloud.google.com/bigquery/docs/bigquery-dataframes-introduction#pricing) .
