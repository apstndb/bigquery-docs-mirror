...
The token quotas apply to BigQuery SQL functions designed for generative AI inference tasks that use Gemini LLMs, such as the [`AI.CLASSIFY`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-ai-classify) and [`AI.GENERATE_TEXT`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-ai-generate-text) functions. The token quotas don't apply to embedding generation and semantic search functions, such as [`AI.EMBED`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-ai-embed) , which have different billing structures.
...
## Important considerations
  ...
  - **Provisioned throughput:** If you are using Gemini Enterprise Agent Platform models with provisioned throughput, billing is not based on token usage. You should set these BigQuery token quotas to a high value to avoid unnecessarily blocking your queries.
