---
name: documents/docs.cloud.google.com/bigquery/docs/information-schema-write-api-by-organization
uri: https://docs.cloud.google.com/bigquery/docs/information-schema-write-api-by-organization
title: WRITE_API_TIMELINE_BY_ORGANIZATION view
description: A fully managed, petabyte-scale analytics data warehouse that lets you run analytics over vast amounts of data in near real time.
data_source: docs.cloud.google.com
---

# WRITE\_API\_TIMELINE\_BY\_ORGANIZATION view

The `INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION` view contains per minute aggregated streaming statistics for the whole organization associated with the current project.
...
To query the `INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION` view, you need the `bigquery.tables.list` Identity and Access Management (IAM) permission for the organization.
...
The `INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION` view has the following schema:
...
| ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------ | ------------ |
| ``[         PROJECT_ID        .]`region-         REGION        `.INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION`` | Organization that contains the specified project | REGION       |
...
The following example calculates the per minute breakdown of total failed requests for all tables in the project's organization in the last 30 minutes, split by error code:
...
The following example calculates a per minute breakdown of successful and failed append requests in the project's organization, split into error code categories. This query could be used to populate a dashboard.
...
The following example returns the BigQuery Storage Write API (gRPC) ingestion statistics for the 10 tables in the project's organization with the most incoming traffic:
