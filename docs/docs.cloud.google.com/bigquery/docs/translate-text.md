...
This document describes how to use the [`  ML.TRANSLATE  ` function](/bigquery/docs/reference/standard-sql/bigqueryml-syntax-translate) with a [remote model](/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-service) to translate text from a [BigQuery standard table](/bigquery/docs/tables-intro#standard-tables) .

## Required roles

To create a remote model and translate text, you need the following Identity and Access Management (IAM) roles at the project level:
...
Create a remote model with a [`  REMOTE_SERVICE_TYPE  `](/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-service#remote_service_type) of `  CLOUD_AI_TRANSLATE_V3  ` :
...
Replace the following:
  ...
  - `  MODE  ` : the name of a supported [translation mode](/bigquery/docs/reference/standard-sql/bigqueryml-syntax-translate#translate-syntax) .
  - `  LANGUAGE  ` : the name of a [supported language code](/translate/docs/languages) . This argument is only required when you use the `  TRANSLATE_TEXT  ` translation mode.
