...
title: BigQuery Storage Write API best practices
...
This document gives best practices for using the BigQuery Storage Write API. Before reading this document, read [Overview of the BigQuery Storage Write API](https://docs.cloud.google.com/bigquery/docs/write-api#overview) .
...
You can also configure the `JsonStreamWriter` to ignore unknown fields in the input data. To set this behavior, call [`setIgnoreUnknownFields`](https://docs.cloud.google.com/java/docs/reference/google-cloud-bigquerystorage/latest/com.google.cloud.bigquery.storage.v1.JsonStreamWriter.Builder#com_google_cloud_bigquery_storage_v1_JsonStreamWriter_Builder_setIgnoreUnknownFields_boolean_) . This behavior is similar to the `ignoreUnknownValues` option when using the legacy [`tabledata.insertAll`](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/tabledata/insertAll) API. However, it can lead to unintentional data loss, because unknown fields are silently dropped.
