...
If you need to securely manage a keyset or transmit it over an untrusted channel, consider using a wrapped keyset. When you wrap a raw keyset, this process encrypts the raw keyset using a [Cloud KMS key](https://docs.cloud.google.com/bigquery/docs/aead-encryption-concepts#cloud_kms_protection) .
...
As with [keysets](https://docs.cloud.google.com/bigquery/docs/aead-encryption-concepts#keysets) , wrapped keysets can, and should, be periodically rotated. Wrapped keysets are used in [AEAD envelope encryption functions](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/aead_encryption_functions) .
