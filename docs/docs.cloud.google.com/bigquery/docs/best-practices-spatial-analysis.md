---
name: documents/docs.cloud.google.com/bigquery/docs/best-practices-spatial-analysis
uri: https://docs.cloud.google.com/bigquery/docs/best-practices-spatial-analysis
...
This document assumes that your BigQuery geospatial tables are [clustered on a geography column](https://docs.cloud.google.com/bigquery/docs/geospatial-data#partitioning_and_clustering_geospatial_data) .
...
Many complex shapes with large numbers of points can be simplified without much loss in precision. Use the BigQuery [`ST_SIMPLIFY`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_simplify) and [`ST_SNAPTOGRID`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_snaptogrid) functions separately or together to reduce the number of points in complex shapes. Combine these functions with BigQuery [materialized views](https://docs.cloud.google.com/bigquery/docs/materialized-views-create) to store a simplified version of your original dataset as a materialized view that's automatically kept up to date against the base table.
...
For example, consider loading a database of land parcels in the United States into a BigQuery table and then determining the nearest body of water. In this case, precomputing parcel centroids using the [`ST_CENTROID`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_centroid) function in combination with the method described in the [Simplify shapes](https://docs.cloud.google.com/bigquery/docs/best-practices-spatial-analysis#simplify-shapes) section of this document can reduce the number of comparisons performed when using the [`ST_DISTANCE`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_distance) or [`ST_DWITHIN`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_dwithin) functions. When using the `ST_CENTROID` function, the parcel centroid needs to be considered in the calculation. Precomputing the parcel centroids in this way can also reduce variability in performance, because different parcel shapes are likely to contain different numbers of points.

A variant of this method is to use the [`ST_BOUNDINGBOX`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_boundingbox) function instead of the `ST_CENTROID` function to compute a rectangular envelope around the input shape. While it's not quite as efficient as using a single point, it can reduce the occurrence of certain edge cases. This variant still offers good and consistent performance, since the output of the `ST_BOUNDINGBOX` function always contains only four points that need to be considered. The bounding box result will be of the type [`STRUCT`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-types#struct_type) , which means you'll need to calculate the distances manually or use the [vector index method](https://docs.cloud.google.com/bigquery/docs/best-practices-spatial-analysis#use-vector-search-and-vector-indexes) described later in this document.
...
You can use the [`ST_CONVEXHULL`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_convexhull) function to optimize for representing the location of the shape. Using this function improves accuracy, but this comes at the cost of decreased performance. The `ST_CONVEXHULL` function is similar to the [`ST_EXTENT`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions#st_extent) function, except the output shape contains more points and varies in the number of points based on the complexity of the input shape. While the performance benefit is likely negligible for small datasets of non-complex shapes, for very large datasets with large and complex shapes, the `ST_CONVEXHULL` function offers a good balance between cost, performance, and accuracy.
...
Using [standard grid systems such as S2 and H3](https://docs.cloud.google.com/bigquery/docs/grid-systems-spatial-analysis#s2_and_h3_grid_systems) can speed up such statistical aggregations and spatial analyses. Using these grid systems can also simplify the development of analytics and improve development efficiency.
