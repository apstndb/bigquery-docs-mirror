---
name: documents/docs.cloud.google.com/bigquery/docs/managed-disaster-recovery
uri: https://docs.cloud.google.com/bigquery/docs/managed-disaster-recovery
...
Disaster recovery uses [Turbo replication](https://docs.cloud.google.com/storage/docs/availability-durability#turbo-replication) for faster data replication across regions, which [reduces the risk of data loss exposure](https://docs.cloud.google.com/architecture/disaster-recovery#step_1_gather_existing_requirements) , minimize service downtime, and [helps support uninterrupted service](https://docs.cloud.google.com/bigquery/docs/reliability-intro#disaster_planning) following a regional outage.

Turbo replication doesn't apply to the initial backfill operation. After the initial backfill operation is completed, turbo replication aims to replicate datasets to a single failover region pair with a secondary replica within 15 minutes, as long as the [bandwidth quota](https://docs.cloud.google.com/bigquery/quotas#bandwidth_limits) isn't exceeded and there are no user errors.
...
A recovery point objective (RPO) is the most recent point in time from which data must be able to be restored. For more information on RPO, see [Basics of DR planning](https://docs.cloud.google.com/architecture/dr-scenarios-planning-guide) . Managed disaster recovery has a RPO that is defined per dataset. The RPO aims to keep the secondary replica within 15 minutes of the primary. To meet this RPO, you can't exceed the [bandwidth quota](https://docs.cloud.google.com/bigquery/quotas#bandwidth_limits) and there can't be any user errors.
