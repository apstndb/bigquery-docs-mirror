...
Alternatively, for capacity compute in BigQuery, you incur charges for the compute resources ( *[slots](https://docs.cloud.google.com/bigquery/docs/slots)* ) that are used to process the query. To use this model, you configure *[reservations](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management)* for slots.
...
To control the costs of individual queries, we recommend that you first follow best practices for [optimizing query computation](https://docs.cloud.google.com/bigquery/docs/best-practices-performance-compute) and [optimizing storage](https://docs.cloud.google.com/bigquery/docs/best-practices-storage) .
...
You can manage costs by setting a [custom quota](https://docs.cloud.google.com/bigquery/docs/custom-quotas) that specifies a limit on the amount of data processed per day per project or per user. Users are not able to run queries once the quota is reached.

To set a custom quota, you need [specific roles or permissions](https://docs.cloud.google.com/bigquery/docs/custom-quotas#required_role) . For quotas to set, see [Quotas and limits](https://docs.cloud.google.com/bigquery/quotas) .
...
Instead, break your query into stages where each stage materializes the query results by writing them to a [destination table](https://docs.cloud.google.com/bigquery/querying-data#permanent-table) . Querying the smaller destination table reduces the amount of data that is read and lowers costs. The cost of storing the materialized results is much less than the cost of processing large amounts of data.
...
To estimate costs in the [Google Cloud pricing calculator](https://cloud.google.com/products/calculator) when using the capacity-based pricing model with [BigQuery editions](https://docs.cloud.google.com/bigquery/docs/editions-intro) , follow these steps:
...
The [BigQuery slot estimator](https://docs.cloud.google.com/bigquery/docs/slot-estimator) helps you to manage slot capacity based on historical performance metrics.
...
To free capacity, check on long-running jobs to make sure that they should continue running. If not, [cancel](https://docs.cloud.google.com/bigquery/docs/managing-jobs#cancel_jobs) them.
...
**Best practice:** Use [Cloud Billing budgets](https://docs.cloud.google.com/billing/docs/how-to/budgets) to monitor your BigQuery charges in one place.
...
Use these best practices for optimizing the cost of BigQuery storage. You can also [optimize storage for query performance](https://docs.cloud.google.com/bigquery/docs/best-practices-storage) .
...
Reducing the [time travel](https://docs.cloud.google.com/bigquery/docs/time-travel) window from the default value of seven days reduces the retention period for data deleted from or changed in a table. You are billed for time travel storage only when using the physical (compressed) [storage billing model](https://docs.cloud.google.com/bigquery/docs/datasets-intro#dataset_storage_billing_models) .

The time travel window is set at the dataset level. You can also set the default time travel window for new datasets using [configuration settings](https://docs.cloud.google.com/bigquery/docs/default-configuration) .
...
Keeping large result sets in BigQuery storage has a cost. If you don't need permanent access to the results, use the [default table expiration](https://docs.cloud.google.com/bigquery/docs/updating-datasets#table-expiration) to automatically delete the data for you.

### Archive data to Cloud Storage

**Best practice:** Consider archiving data in Cloud Storage.

You can move data from BigQuery to Cloud Storage based on the business need for archival. As a best practice, consider [long-term storage pricing](https://docs.cloud.google.com/bigquery/docs/best-practices-costs#store-data-bigquery) and the [physical storage billing model](https://docs.cloud.google.com/bigquery/docs/best-practices-costs#storage-billing-model) before [exporting data out of BigQuery](https://docs.cloud.google.com/bigquery/docs/exporting-data) .
