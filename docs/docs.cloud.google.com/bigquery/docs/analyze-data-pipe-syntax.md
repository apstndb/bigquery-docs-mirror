---
name: documents/docs.cloud.google.com/bigquery/docs/analyze-data-pipe-syntax
uri: https://docs.cloud.google.com/bigquery/docs/analyze-data-pipe-syntax
title: Analyze data using pipe syntax
...
In pipe syntax, the query can start with a [`FROM` clause](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/pipe-syntax#from_queries) without a `SELECT` clause to return table results.
...
In the `bikeshare_trips` table, the `start_time` column is a timestamp, but you might want to add a column that only shows the date of the trip. To add a column in pipe syntax, use the [`EXTEND` pipe operator](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/pipe-syntax#extend_pipe_operator) :
...
To sort the results in descending order by the `date` column, add the [`DESC`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/pipe-syntax#shorthand_order_pipe_syntax) suffix to the `GROUP BY` clause:
...
In pipe syntax, you can add the sorting suffix directly to the `GROUP BY` clause without using the [`ORDER BY` pipe operator](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/pipe-syntax#order_by_pipe_operator) . Adding the suffix to the `GROUP BY` clause is one of several optional [shorthand ordering features with `AGGREGATE`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/pipe-syntax#shorthand_order_pipe_syntax) that pipe syntax supports. In standard syntax, this isn't possible and you must use the `ORDER BY` clause for sorting.
...
To update the rows in your table to display weeks instead of days, use the [`DATE_TRUNC` function](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/date_functions#date_trunc) in the `GROUP BY` clause and set the granularity to `WEEK` :
