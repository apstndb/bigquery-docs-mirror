---
name: documents/docs.cloud.google.com/bigquery/docs/search-intro
uri: https://docs.cloud.google.com/bigquery/docs/search-intro
...
There is no charge for the processing required to build and refresh your search indexes when the total size of indexed tables in your organization is below your region's [limit](https://docs.cloud.google.com/bigquery/quotas#index_limits) . To support indexing beyond this limit, you need to [provide your own reservation](https://docs.cloud.google.com/bigquery/docs/search-index#use_your_own_reservation) for handling the index-management jobs. Search indexes incur storage costs when they are active. You can find the index storage size in the [`INFORMATION_SCHEMA.SEARCH_INDEXES` view](https://docs.cloud.google.com/bigquery/docs/information-schema-indexes) .
