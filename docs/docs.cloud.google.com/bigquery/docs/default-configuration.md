...
Use the following settings to define rules for data creation, security, and lifecycle.
  ...
  - `default_data_policy_projects` : The default list of projects whose [data policies](https://docs.cloud.google.com/bigquery/docs/column-level-security-intro#data_policies) are evaluated when accessing columns protected by [data governance tags](https://docs.cloud.google.com/bigquery/docs/tags#data-governance-tags) . By default, only the data policies of the table's project are evaluated. When you configure a default data policy project at the organization level, BigQuery evaluates both the data policies of the table's project and those of the default data policy project. If conflicting policies exist in both projects, the data policies in the table's project take precedence. This setting can only be configured at the organization level using the [`ALTER ORGANIZATION SET OPTIONS` statement](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language#alter_organization_set_options_statement) (for example, `region-us.default_data_policy_projects = ["my_default_project"]` ). For more information, see [Configure a default data policy project](https://docs.cloud.google.com/bigquery/docs/tags#configure-default-data-policy-project) .
...
Use the following settings to determine how resources are billed and connected.
  ...
  - `preflight_fluid_autoscaling_reservations` : The list of reservations that are opted in to fluid scaling for per-second billing with no minimum duration. For more information, see [Understand reservations](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management) .
