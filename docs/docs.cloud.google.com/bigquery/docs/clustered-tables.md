---
name: documents/docs.cloud.google.com/bigquery/docs/clustered-tables
uri: https://docs.cloud.google.com/bigquery/docs/clustered-tables
...
In BigQuery, a *clustered column* is a user-defined table property that sorts [storage blocks](https://docs.cloud.google.com/bigquery/docs/storage_overview#storage_layout) based on the values in the clustered columns. The storage blocks are adaptively sized based on the size of the table. Colocation occurs at the level of the storage blocks, and not at the level of individual rows; for more information on colocation in this context, see [Clustering](https://docs.cloud.google.com/bigquery/docs/migration/schema-data-overview#clustering) .
...
If a clustered table is referenced multiple times in a query that uses several filters, BigQuery charges for scanning the columns in the appropriate blocks in each of the respective filters. For an example of how block pruning works, see [Example](https://docs.cloud.google.com/bigquery/docs/clustered-tables#example) .

## Combine clustered and partitioned tables

You can combine table clustering with [table partitioning](https://docs.cloud.google.com/bigquery/docs/partitioned-tables) to achieve finely-grained sorting for further query optimization.
...
BigQuery restricts the use of shared Google Cloud resources with [quotas and limits](https://docs.cloud.google.com/bigquery/quotas) , including limitations on certain table operations or the number of jobs run within a day.

When you use the clustered table feature with a partitioned table, you are subject to the [limits on partitioned tables](https://docs.cloud.google.com/bigquery/quotas#partitioned_tables) .

Quotas and limits also apply to the different types of jobs that you can run against clustered tables. For information about the job quotas that apply to your tables, see [Jobs](https://docs.cloud.google.com/bigquery/quotas#jobs) in "Quotas and Limits".
...
Like other BigQuery table operations, clustered table operations take advantage of BigQuery free operations such as batch load, table copy, automatic reclustering, and data export. These operations are subject to [BigQuery quotas and limits](https://docs.cloud.google.com/bigquery/quotas) . For information about free operations, see [Free operations](https://cloud.google.com/bigquery/pricing#free) .

For a detailed clustered table pricing example, see [Estimate storage and query costs](https://docs.cloud.google.com/bigquery/docs/estimate-costs#clustered_table_pricing_example) .
