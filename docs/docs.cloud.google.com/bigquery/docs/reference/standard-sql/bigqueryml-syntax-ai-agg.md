...
We recommend the `  AI.AGG  ` function for aggregation tasks instead of the [`  AI.GENERATE  `](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-ai-generate) and [`  AI.GENERATE_TEXT  `](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-ai-generate-text) general-purpose AI functions for the following reasons:
...
The `  AI.AGG  ` function takes the following arguments:
  ...
  - `  INPUT  ` : the data to be aggregated, as either a `  STRING  ` value or a `  STRUCT  ` object consisting of `  STRING  ` values, [`  ObjectRefRuntime  ` values](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/objectref_functions#objectrefruntime) , and arrays of `  STRING  ` and `  ObjectRefRuntime  ` values. `  ObjectRefRuntime  ` values reference text or image data in Cloud Storage and are generated by the [`  OBJ.GET_ACCESS_URL  ` function](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/objectref_functions#objget_access_url) . Text and image data must meet the [technical specifications](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/models/gemini/2-5-flash#technical-specifications) for the version of Gemini that you plan to use.
  ...
  - `  INSTRUCTION  ` : the natural language aggregation prompt to apply to the input data, as either a string or [query parameter](https://docs.cloud.google.com/bigquery/docs/parameterized-queries) .
...
The `  AI.AGG  ` function has the following known issues:
  ...
  - Queries that use Gemini 3.0 or 3.1 with connection-based authentication might receive an unauthenticated error. To resolve this issue, use [end-user credentials](https://docs.cloud.google.com/bigquery/docs/permissions-for-ai-functions#run_generative_ai_queries_with_end-user_credentials) .
