...
`  AI.GENERATE_DOUBLE  ` returns a `  STRUCT  ` value for each row in the table. The struct contains the following fields:
  ...
  - `  result  ` : a `  FLOAT64  ` value containing the model's response to the prompt. The result is `  NULL  ` if the request fails or is filtered by [responsible AI](/vertex-ai/generative-ai/docs/learn/responsible-ai) .
...
You can run `  AI.GENERATE_DOUBLE  ` in all of the [regions](/vertex-ai/generative-ai/docs/learn/locations#google_model_endpoint_locations) that support Gemini models, and also in the `  US  ` and `  EU  ` multi-regions.
