...
You can use DNN regressor models with the [`  ML.PREDICT  ` function](/bigquery/docs/reference/standard-sql/bigqueryml-syntax-predict) to perform [regression](/bigquery/docs/regression-overview) , and you can use DNN classifier models with the `  ML.PREDICT  ` function to perform [classification](/bigquery/docs/classification-overview) . You can use both types of DNN models with the `  ML.PREDICT  ` function to perform [anomaly detection](/bigquery/docs/anomaly-detection-overview) .
...
Specifies the number of steps to sample between the example being explained and its baseline for approximating the integral when using [integrated gradients](/ai-platform/prediction/docs/ai-explanations/overview#ig) attribution methods.
...
The hyperparameter tuning objective for the model; only one objective is supported. If you specify a value for this option, then you must also specify a value for [`  NUM_TRIALS  `](#num_trials) .
...
DNN classification and regression models support [hyperparameter tuning](/bigquery/docs/hp-tuning-overview) , which you can use to improve model performance for your data. To use hyperparameter tuning, set the [`  NUM_TRIALs  ` option](#num_trials) to the number of trials that you want to run. BigQuery ML then trains the model the number of times that you specify, using different hyperparameter values, and returns the model that performs the best.
...
For more information about the training objectives and hyperparameters supported for DNN classification models, see [`  DNN_CLASSIFIER  `](/bigquery/docs/hp-tuning-overview#dnn_classifier) . For more information about the training objectives and hyperparameters supported for DNN regression models, see [`  DNN_REGRESSOR  `](/bigquery/docs/hp-tuning-overview#dnn_regressor) . To try a tutorial that walks you through hyperparameter tuning, see [Improve model performance with hyperparameter tuning](/bigquery/docs/hyperparameter-tuning-tutorial) .
...
Training DNN models is not supported in all BigQuery ML regions. For a complete list of supported regions and multi-regions, see [BigQuery ML locations](/bigquery/docs/locations#bqml-loc) page.
