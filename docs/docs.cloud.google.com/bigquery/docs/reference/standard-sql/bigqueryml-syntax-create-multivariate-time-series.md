---
name: documents/docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-multivariate-time-series
uri: https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-multivariate-time-series
title: The CREATE MODEL statement for ARIMA_PLUS_XREG models
description: Use the CREATE MODEL statement for creating multivariate time series models in BigQuery.
...
The amount of [L2 regularization](https://developers.google.com/machine-learning/glossary/#L2_regularization) applied in the linear regression.
...
The following example models only the custom holiday `members_day` for the `US` holiday region:
    ...
    CREATE OR REPLACE MODEL mydataset.arima_model  OPTIONS (    model_type = 'ARIMA_PLUS_XREG',    -- Don't specify HOLIDAY_REGION    ...) AS (        training_data AS (SELECT * FROM mydataset.timeseries_data),        custom_holiday AS (            SELECT              'US' AS region,              'members_day' AS holiday_name,              primary_date,              2 AS preholiday_days,              2 AS postholiday_days            FROM              UNNEST(                [                  DATE('2016-06-15'),                  DATE('2017-06-07'),                  DATE('2018-06-06')])                AS primary_date          )      );
...
The following example models all built-in holidays for the `US` holiday region except for the `Christmas` and `NewYears` holidays:
    ...
    CREATE OR REPLACE MODEL mydataset.arima_model  OPTIONS (    model_type = 'ARIMA_PLUS_XREG',    -- Don't specify HOLIDAY_REGION    ...) AS (        training_data AS (SELECT * FROM mydataset.timeseries_data),        custom_holiday AS (            SELECT *            FROM bigquery-public-data.ml_datasets.holiday            WHERE              region = 'US'              AND (holiday_name != 'Christmas' OR holiday_name != 'NewYear')          )      );
...
`ARIMA_PLUS_XREG` models have the following limitations:
  ...
  - You can't export `ARIMA_PLUS_XREG` models.

## Examples

The following examples show how to create different types of `ARIMA_PLUS_XREG` time series models.
...
### Train a time series model more quickly by using a fraction of the time points
