...
`AuditData` is the older log format. The `protoPayload.serviceData` field isn't populated for new features or log entries. For new use cases, use the [`BigQueryAuditMetadata` format](https://docs.cloud.google.com/bigquery/docs/reference/auditlogs#bigqueryauditmetadata_format) .
...
When you run a multi-statement query or call a stored procedure, BigQuery runs each statement as a separate [child job](https://docs.cloud.google.com/bigquery/docs/multi-statement-queries#enumerate_child_jobs_of_a_multi-statement_query) . The parent job generates `JobInsertion` and `JobChange` events, but it doesn't read table data. The `TableDataRead` and `TableDataChange` events are generated by the child jobs. To trace data access back to the parent job, use the `jobStats.parentJobName` field, which is set on the audit log entries of each child job. Child job IDs are also prefixed with `script_job_` .

**Note:** BigQuery doesn't emit data access log entries in the following scenarios:

  - If a job fails before or during execution, `TableDataChange` and `TableDataRead` events aren't logged.

  - Audit log entries aren't emitted for every job. For example, events like `jobCompletedEvent` only capture successful job completions. Disruptions or errors during job execution or log generation can prevent audit log entries from being written. To track or count all submitted jobs regardless of completion status, use [`INFORMATION_SCHEMA` views](https://docs.cloud.google.com/bigquery/docs/information-schema-intro) , such as [`INFORMATION_SCHEMA.JOBS`](https://docs.cloud.google.com/bigquery/docs/information-schema-jobs) .
  ...
  - Query results that are served from [cached query results](https://docs.cloud.google.com/bigquery/docs/cached-results) don't generate `TableDataRead` entries, because the referenced tables aren't read.

  - The parent job of a multi-statement query or a stored procedure call doesn't generate `TableDataRead` or `TableDataChange` entries. These entries are generated by the [child jobs](https://docs.cloud.google.com/bigquery/docs/multi-statement-queries#enumerate_child_jobs_of_a_multi-statement_query) that run the individual statements.
...
After the sink is created, give the service account created by the previous command [access to the dataset](https://docs.cloud.google.com/bigquery/docs/control-access-to-resources-iam#grant_access_to_a_dataset) .
