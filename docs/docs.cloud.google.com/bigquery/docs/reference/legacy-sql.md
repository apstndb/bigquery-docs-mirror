---
name: documents/docs.cloud.google.com/bigquery/docs/reference/legacy-sql
uri: https://docs.cloud.google.com/bigquery/docs/reference/legacy-sql
title: Legacy SQL Syntax, Functions and Operators
...
**Notes:**
  ...
  - Expressions computed in the `SELECT` clause cannot be used in the corresponding `GROUP BY` clause.
        ...
        #legacySQLSELECT  word,  corpus,  COUNT(word) word_countFROM  [bigquery-public-data:samples.shakespeare]WHERE  word CONTAINS "th"GROUP BY  word,  corpus,  word_count;  /* Fails because word_count is not visible to this GROUP BY clause. */
