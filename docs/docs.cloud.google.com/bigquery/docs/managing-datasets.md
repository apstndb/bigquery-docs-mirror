...
As a BigQuery administrator, you can organize and control access to [tables](https://docs.cloud.google.com/bigquery/docs/tables) and [views](https://docs.cloud.google.com/bigquery/docs/views) that analysts use. For more information about datasets, see [Introduction to datasets](https://docs.cloud.google.com/bigquery/docs/datasets-intro) .

You cannot change the name of an existing dataset or relocate a dataset after it's created. As a workaround for changing the dataset name, you can [copy a dataset](https://docs.cloud.google.com/bigquery/docs/managing-datasets#copy-datasets) and change the destination dataset's name. To relocate a dataset, you can follow one of the following methods:
...
The following limitations apply when you copy datasets:
  ...
  - Copying a dataset is not supported in [BigQuery Omni regions](https://docs.cloud.google.com/bigquery/docs/omni-introduction#locations) .

  - To copy a dataset to a project in another [VPC Service Controls service perimeter](https://docs.cloud.google.com/vpc-service-controls/docs/service-perimeters) , you need to set the following egress rules:
...
3.  [Create a BigQuery dataset](https://docs.cloud.google.com/bigquery/docs/datasets) in the same region or a different region from your source dataset.
...
1.  [Export the data](https://docs.cloud.google.com/bigquery/docs/exporting-data) from your BigQuery tables to a Cloud Storage bucket.
...
You can restore tables from a deleted dataset that are within the dataset's [time travel window](https://docs.cloud.google.com/bigquery/docs/time-travel) . To restore the entire dataset, see [Restore deleted datasets](https://docs.cloud.google.com/bigquery/docs/restore-deleted-datasets) .
...
To learn how to restore (or *undelete* ) a deleted dataset, see [Restore deleted datasets](https://docs.cloud.google.com/bigquery/docs/restore-deleted-datasets) .
