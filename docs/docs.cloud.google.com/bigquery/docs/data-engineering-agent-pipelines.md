...
To produce higher quality agent responses, the Data Engineering Agent can retrieve additional data and metadata from BigQuery and Knowledge Catalog, including sample rows from BigQuery tables and data scan profiles generated in Knowledge Catalog. This data is not used to train the data engineering agent, and is only used during agent conversations as additional context to inform the agent's responses.
