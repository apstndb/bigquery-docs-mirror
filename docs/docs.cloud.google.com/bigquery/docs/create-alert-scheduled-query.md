---
name: documents/docs.cloud.google.com/bigquery/docs/create-alert-scheduled-query
uri: https://docs.cloud.google.com/bigquery/docs/create-alert-scheduled-query
...
Create and run a SQL query in BigQuery that generates the output for your alert. The query captures the logic that you want to monitor. For more information, see [Run a query](https://docs.cloud.google.com/bigquery/docs/running-queries) .

## Set up a scheduled query

You can schedule queries to run on a recurring basis, from every 15 minutes to every several months. You can write any query over your log buckets. For more information, see [Scheduling queries](https://docs.cloud.google.com/bigquery/docs/scheduling-queries) .
...
Use a [metric-threshold alert](https://docs.cloud.google.com/monitoring/alerts/using-alerting-ui) to detect when the number of rows returned by the scheduled query differs from a threshold.
