...
If you have a private Amazon Redshift instance in AWS, you can migrate that data to BigQuery by creating a [virtual private cloud (VPC) network](https://docs.cloud.google.com/vpc/docs/overview) and connecting it with the Amazon Redshift VPC network. The data migration process works as follows:
...
7.  In the **Data source details** section, do the following:
    ...
    5.  For **Amazon S3 URI** , enter the [URI of the S3 bucket](https://docs.cloud.google.com/bigquery/docs/migration/redshift-vpc#s3_uri) you'll use as a staging area.
