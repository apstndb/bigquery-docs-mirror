...
BigQuery continuously calculates how many slots are required by queries as they execute, but it allocates slots to queries based on a [fair scheduler](https://docs.cloud.google.com/bigquery/docs/slots#fair_scheduling_in_bigquery) .
...
BigQuery uses [IAM](https://docs.cloud.google.com/iam) to manage access to resources and provides centralized [access management](https://docs.cloud.google.com/bigquery/docs/access-control) to resources and actions. The types of resources available in BigQuery include organizations, projects, datasets, tables, and views. In the IAM policy hierarchy, datasets are child resources of projects. A table inherits permissions from the dataset that contains it.
...
To achieve row-level security in BigQuery, you can use [authorized views](https://docs.cloud.google.com/bigquery/docs/authorized-views) and [row-level access policies](https://docs.cloud.google.com/bigquery/docs/managing-row-level-security) . For more information about how to design and implement these policies, see [Introduction to BigQuery row-level security](https://docs.cloud.google.com/bigquery/docs/row-level-security-intro) .
...
BigQuery encrypts all data [at rest](https://docs.cloud.google.com/docs/security/encryption/default-encryption) and [in transit](https://docs.cloud.google.com/docs/security/encryption-in-transit) by default regardless of the source or any other condition, and this cannot be turned off. BigQuery also supports [customer-managed encryption keys](https://docs.cloud.google.com/bigquery/docs/customer-managed-encryption) (CMEK) for users who want to control and manage key encryption keys in [Cloud Key Management Service](https://docs.cloud.google.com/kms/docs) . For more information about encryption at Google Cloud, see [Default encryption at rest](https://docs.cloud.google.com/docs/security/encryption/default-encryption) and [Encryption in transit](https://docs.cloud.google.com/docs/security/encryption-in-transit) .
...
BigQuery supports [dynamic data masking](https://docs.cloud.google.com/bigquery/docs/column-data-masking-intro) at the column level. You can use data masking to selectively obscure column data for groups of users, while still allowing access to the column.

You can use the [Sensitive Data Protection](https://docs.cloud.google.com/sensitive-data-protection/docs) to identify and redact sensitive personally identifiable information (PII) on BigQuery.
...
BigQuery operates directly on compressed data without decompressing by using Capacitor. BigQuery provides [datasets](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/datasets) as the highest-level abstraction to organize access to tables as shown in the preceding diagram. [Schemas](https://docs.cloud.google.com/bigquery/docs/schemas) and [labels](https://docs.cloud.google.com/bigquery/docs/adding-using-labels) can be used for further organization of tables. BigQuery offers [partitioning](https://docs.cloud.google.com/bigquery/docs/partitioned-tables) to improve query performance and costs and to manage information lifecycle. Storage resources are allocated as you consume them and deallocated as you remove data or drop tables.
...
BigQuery offers per-user [cache](https://docs.cloud.google.com/bigquery/docs/cached-results) , and if data doesn't change, results of queries are cached for approximately 24 hours. If the results are retrieved from the cache, the query costs nothing.
...
BigQuery datasets, tables, and views can be [labeled](https://docs.cloud.google.com/bigquery/docs/adding-using-labels) with key-value pairs. Labels can be used for differentiating storage costs and internal chargebacks.
...
For BigQuery, [Cloud Audit Logs](https://docs.cloud.google.com/logging/docs/audit) is used for both data access logs and audit logs, which are enabled by default. The data access logs are available for 30 days, and the other system events and admin activity logs are available for 400 days. If you need longer retention, you can [export](https://docs.cloud.google.com/solutions/design-patterns-for-exporting-stackdriver-logging) logs to BigQuery, Cloud Storage, or Pub/Sub as described in [Security log analytics in Google Cloud](https://docs.cloud.google.com/solutions/design-patterns-for-exporting-stackdriver-logging) . If integration with an existing incident monitoring tool is needed, Pub/Sub can be used for exports, and custom development should be done on the existing tool to read logs from Pub/Sub.

Audit logs include all the API calls, query statements, and job statuses. You can use [Cloud Monitoring](https://docs.cloud.google.com/bigquery/docs/monitoring) to monitor slot allocation, bytes scanned in queries and stored, and other BigQuery [metrics](https://docs.cloud.google.com/bigquery/docs/monitoring#metrics) . BigQuery [query plan and timeline](https://docs.cloud.google.com/bigquery/query-plan-explanation) can be used to analyze query stages and performance.

![The query plan.](https://docs.cloud.google.com/static/bigquery/images/explanation-button.png)

You can use the [error messages table](https://docs.cloud.google.com/bigquery/troubleshooting-errors) for troubleshooting query job and API errors. To distinguish slot allocations per query or job, you can use this [utility](https://github.com/GoogleCloudPlatform/professional-services/tree/master/examples/bigquery-cross-project-slot-monitoring) , which is beneficial for customers that use capacity-based pricing and have many projects distributed across several teams.

#### Maintenance, upgrades, and versions

BigQuery is a fully managed service and does not require you to do any maintenance or upgrades. BigQuery does not offer different versions. Upgrades are continuous and don't require downtime or hinder system performance. For more information, see [Release notes](https://docs.cloud.google.com/bigquery/docs/release-notes) .
...
Oracle has hard and soft limits based on infrastructure, hardware capacity, parameters, software versions, and licensing. BigQuery has [quotas and limits](https://docs.cloud.google.com/bigquery/quotas) on specific actions and objects.
...
Oracle data types differ from BigQuery data types. For more information about BigQuery data types, refer to the official [documentation](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-types) .

For a detailed comparison between Oracle and BigQuery data types, see the [Oracle SQL translation guide](https://docs.cloud.google.com/bigquery/docs/migration/oracle-sql) .

### Indexes

In many analytical workloads, columnar tables are used instead of row stores. This greatly increases the column-based operations and eliminates the use of indexes for batch analytics. BigQuery also stores data in a columnar format, so indexes are not needed in BigQuery. If the analytics workload requires a single, small set of row-based access, [Bigtable](https://docs.cloud.google.com/bigtable/docs/overview#top_of_page) can be a better alternative. If a workload requires transaction processing with strong relational consistencies, [Spanner](https://docs.cloud.google.com/spanner/docs/overview) or [Cloud SQL](https://docs.cloud.google.com/sql/docs) can be better alternatives.

To summarize, no indexes are needed and offered in BigQuery for batch analytics. [Partitioning](https://docs.cloud.google.com/bigquery/docs/partitioned-tables) or [clustering](https://docs.cloud.google.com/bigquery/docs/clustered-tables) can be used. For more information about how to tune and improve query performance in BigQuery, see [Introduction to optimizing query performance](https://docs.cloud.google.com/bigquery/docs/best-practices-performance-overview) .

### Views

Similar to Oracle, BigQuery allows creating custom views. However, [views in BigQuery](https://docs.cloud.google.com/bigquery/docs/views-intro) don't support DML statements.
...
BigQuery uses temporary tables to cache query results that aren't written to a permanent table. After a query finishes, the temporary tables exist for up to 24 hours. The tables are created in a special dataset and named randomly. You can also create temporary tables for your own use. For more information, see [Temporary tables](https://docs.cloud.google.com/bigquery/docs/multi-statement-queries#temporary_tables) .
...
In some cases, denormalization using nested and repeated fields does not lead to performance improvements. For more information about limitations and restrictions, see [Specify nested and repeated columns in table schemas](https://docs.cloud.google.com/bigquery/docs/nested-repeated) .
...
BigQuery supports [column-](https://docs.cloud.google.com/bigquery/docs/column-level-security-intro) and [row-level security](https://docs.cloud.google.com/bigquery/docs/row-level-security-intro) . Column-level security provides fine-grained access to sensitive columns using policy tags, or type-based classification of data. Row-level security which lets you filter data and enables access to specific rows in a table based on qualifying user conditions.
...
[Dataflow](https://docs.cloud.google.com/dataflow) is a serverless data processing platform that can autoscale as well as do batch and streaming data processing. Dataflow can be a good choice for Python and Java developers who want to code their data pipelines and use the same code for both streaming and batch workloads. Use the [JDBC to BigQuery template](https://docs.cloud.google.com/dataflow/docs/guides/templates/provided-batch#jdbctobigquery) to extract data from your Oracle or other relational databases and load it into BigQuery.

##### Managed Service for Apache Airflow

[Managed Airflow](https://docs.cloud.google.com/composer) is Google Cloud fully managed workflow orchestration service built on [Apache Airflow](https://airflow.apache.org/) . It lets you author, schedule, and monitor pipelines that span across cloud environments and on-premises data centers. Managed Airflow provides [operators](https://airflow.apache.org/docs/apache-airflow/1.10.14/_api/index.html#operators) and [contributions](https://airflow.apache.org/docs/apache-airflow/1.10.14/_api/airflow/contrib/operators/index.html) that can run multi-cloud technologies for use cases including extract and loads, transformations of ELT, and REST API calls.

Managed Airflow uses directed acyclic graphs (DAGs) for scheduling and orchestrating workflows. To understand the general airflow concepts, see [Airflow Apache concepts](https://airflow.apache.org/concepts.html) . For more information about DAGs, see [Writing DAGs (workflows)](https://docs.cloud.google.com/composer/docs/how-to/using/writing-dags) . For sample ETL best practices with Apache Airflow, see [ETL best practices with Airflow documentation site¶](https://gtoonstra.github.io/etl-with-airflow/) . You can replace the Hive operator in that example with the [BigQuery operator](https://airflow.apache.org/docs/apache-airflow-providers-google/stable/operators/cloud/bigquery.html) , and the same concepts would be applicable.
...
ETL tools have been around for many years. Some organizations might find it convenient to leverage an existing investment in trusted ETL scripts. Some of our key partner solutions are included on [BigQuery partner website](https://docs.cloud.google.com/bigquery#section-6) . Knowing when to choose partner tools over Google Cloud built-in utilities depends on your current infrastructure and your IT team's comfort with developing data pipelines in Java or Python code.

### Business intelligence (BI) tool migration

BigQuery supports a [flexible suite](https://docs.cloud.google.com/solutions/business-intelligence) of business intelligence (BI) solutions for Reporting and analysis that you can leverage. For more information about BI tool migration and BigQuery integration, see [Overview of BigQuery analytics](https://docs.cloud.google.com/bigquery/docs/query-overview) .

### Query (SQL) translation

BigQuery's [GoogleSQL](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql) supports compliance with the SQL 2011 standard and has extensions that support querying nested and repeated data. All ANSI-compliant SQL functions and operators can be used with minimal modifications. For a detailed comparison between Oracle and BigQuery SQL syntax and functions, see the [Oracle to BigQuery SQL translation reference](https://docs.cloud.google.com/bigquery/docs/migration/oracle-sql) .

Use [batch SQL translation](https://docs.cloud.google.com/bigquery/docs/batch-sql-translator) to migrate your SQL code in bulk, or [interactive SQL translation](https://docs.cloud.google.com/bigquery/docs/interactive-sql-translator) to translate ad hoc queries.
...
BigQuery ML enables developers to run many different types of models, such as linear regression, binary logistic regression, multiclass logistic regression, k-means clustering, and TensorFlow model imports. For more information, see [Introduction to BigQuery ML](https://docs.cloud.google.com/bigquery/docs/bigqueryml-intro) .

Converting ODM jobs might require rewriting the code. You can choose from comprehensive [Google AI product offerings](https://cloud.google.com/products/ai/) such as [BigQuery ML](https://docs.cloud.google.com/bigquery/docs/bqml-introduction) , AI APIs ( [Speech-to-Text](https://docs.cloud.google.com/speech-to-text) , [Text-to-Speech](https://docs.cloud.google.com/text-to-speech) , [Dialogflow](https://docs.cloud.google.com/dialogflow) , [Cloud Translation](https://docs.cloud.google.com/translate/docs) , [Cloud Natural Language API](https://docs.cloud.google.com/natural-language) , [Cloud Vision](https://docs.cloud.google.com/vision) , and more), or [Gemini Enterprise Agent Platform](https://docs.cloud.google.com/vertex-ai) .

[Vertex AI Workbench](https://docs.cloud.google.com/gemini-enterprise-agent-platform/notebooks/workbench/introduction) can be used as a development environment for data scientists, and [Vertex AI Training](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/training-overview) can be used to run training and scoring workloads at scale.
...
BigQuery offers [geospatial analytics](https://docs.cloud.google.com/bigquery/docs/gis) functions and data types. For more information, see [Working with](https://docs.cloud.google.com/bigquery/docs/gis-data) [geospatial analytics data](https://docs.cloud.google.com/bigquery/docs/gis-data) . Oracle Spatial data types and functions can be converted to [geography](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions) [functions in BigQuery standard SQL](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/geography_functions) . Geography functions don't add cost on top of standard BigQuery pricing.

#### Graph

[JanusGraph](https://docs.janusgraph.org/) is an open source graph database solution that can use [Bigtable](https://docs.cloud.google.com/bigtable) as a storage backend. For more information, see [Running JanusGraph on GKE with Bigtable](https://docs.cloud.google.com/solutions/running-janusgraph-with-bigtable) .
