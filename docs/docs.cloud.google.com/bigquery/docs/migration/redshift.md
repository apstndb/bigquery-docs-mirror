---
name: documents/docs.cloud.google.com/bigquery/docs/migration/redshift
uri: https://docs.cloud.google.com/bigquery/docs/migration/redshift
title: Migrate schema and data from Amazon Redshift
description: Provides step-by-step instructions for using the BigQuery Data Transfer Service to migrate schema and data from Amazon Redshift to BigQuery.
...
Add the following Google-owned IP ranges to the inbound rules of your cluster's security group by [configuring the security group rules](https://docs.aws.amazon.com/vpc/latest/userguide/working-with-security-group-rules.html) . You can add the IP addresses that correspond to your dataset's location, or you can add all of the IP addresses in the following table. These Google-owned IP addresses are reserved for Amazon Redshift data migrations.
...
Gather the information that you need to set up the migration with the BigQuery Data Transfer Service:
  ...
  - Follow the instructions at [Grant access to your Amazon S3 bucket](https://docs.cloud.google.com/bigquery/docs/migration/redshift#grant_access_to_your_S3_bucket) to get an AWS access key pair.
