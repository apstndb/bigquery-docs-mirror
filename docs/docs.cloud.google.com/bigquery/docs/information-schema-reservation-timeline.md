---
name: documents/docs.cloud.google.com/bigquery/docs/information-schema-reservation-timeline
uri: https://docs.cloud.google.com/bigquery/docs/information-schema-reservation-timeline
title: RESERVATIONS_TIMELINE view
description: Query BigQuery `INFORMATION_SCHEMA.RESERVATIONS_TIMELINE` for real-time, per-minute reservation metadata and autoscale details.
...
<li><code dir="ltr" translate="no">slots_max_assigned</code> : the maximum slot capacity for this reservation, including slot sharing. If <code dir="ltr" translate="no">ignore_idle_slots</code> is true, this field is the same as <code dir="ltr" translate="no">slots_assigned</code> . Otherwise this field is either the total number of slots in all capacity commitments or in all baselines in the administration project, whichever is larger.</li>
<li><code dir="ltr" translate="no">borrowed_slots</code> : the number of slots used from the idle slot sharing. Only populated if <code dir="ltr" translate="no">ignore_idle_slots</code> is false, and idle slots were used during this second.</li>
<li><code dir="ltr" translate="no">lent_slots</code> : the number of slots other reservations use from the pool of baseline slots of this reservation. Only populated if <code dir="ltr" translate="no">ignore_idle_slots</code> is false, and idle slots were used by other reservations during this second.</li>
</ul>
